...
def render():
    ...
    if st.button("🚨 Submit Incident Report", type="primary", use_container_width=True):
        if not desc.strip():
            ...
            reported_by = st.session_state.get("username", "user")
            ...
            st.markdown(f"""
                <div class="hub-card" style="margin-top: 15px;">
                    ...
                    <p style="color: #F0F6FC; margin: 0; font-weight: 600;">Reference ID: <span style="color: #00E5FF;">#{incident_id}</span></p>
                    <p style="color: #F0F6FC; margin: 4px 0 0 0; font-weight: 600;">Category: <span style="color: {color};">{incident_type}</span></p>
                    <p style="color: #C9D1D9; margin: 5px 0 0 0; font-size: 0.9rem;">"{desc}"</p>
                </div>
            """, unsafe_allow_html=True)

    # Recent incident history from SQLite
    with get_db() as conn:
        incidents = models.list_incidents(conn, limit=10)

    if incidents:
        st.markdown("<hr class='hub-divider'>", unsafe_allow_html=True)
        st.markdown("<h4 style='color: #00E5FF; margin-bottom: 12px;'>📋 Recent Incident Log</h4>", unsafe_allow_html=True)
        ...
        history_rows = [
            {
                ...
                "Severity": i["severity"],
                "Description": i["description"][:80] + ("…" if len(i["description"]) > 80 else ""),
                "Reported By": i["reported_by"] or "—",
                "Logged At": i["created_at"],
            }
            for i in incidents
        ]
        st.dataframe(
